...
import MDAvatar_ from "components/MDAvatar";
import MDBadge_ from "components/MDBadge";

const MDBox = MDBox_ as FC<PropsWithChildren<any>>;
const MDAvatar = MDAvatar_ as FC<PropsWithChildren<any>>;
const MDTypography = MDTypography_ as FC<PropsWithChildren<any>>;
const MDBadge = MDBadge_ as FC<PropsWithChildren<any>>;
...
export function missionData() {
  const Author: FC<any> = ({ image, name, email }) => (
  ...
  const Job: FC<any> = ({ title, description }) => (
  ...
  return {
    ...
    rows: [
      ...
      {
        author: <Author name="Miriam" email="miriam@creative-tim.com" />,
        function: <Job title="Programator" description="Developer" />,
        status: (
          <MDBox ml={-1}>
            <MDBadge badgeContent="offline" color="dark" variant="gradient" size="sm" />
