...
export const TurnByTurnTagsInput: FC<TurnByTurnTagsInputProps> = ({ initialValue, onChange }) => {
  return (
    <>
      ...
      <MultiInput initialValue={initialValue[0].map(r => r.name)} onChange={(tags) => onChange(0, tags)}></MultiInput>
      <Typography>Dialogue Tags2</Typography>
      <MultiInput initialValue={initialValue[1].map(r => r.name)} onChange={(tags) => onChange(1, tags)}></MultiInput>
    </>
  );
};
